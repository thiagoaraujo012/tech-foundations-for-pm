...
export default function OnboardingModal({ onDone }: { onDone: () => void }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.75)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}>
      <div className="ob-card" style={{ maxHeight: '90vh', overflowY: 'auto' }}>
        ...
        <form onSubmit={handleSubmit} className="ob-form">
          ...
          <div className="ob-field">
            ...
            <div className="ob-pill-row">
              <button type="button" className={`ob-pill ${isPM === true ? 'ob-pill-active' : ''}`} onClick={() => setIsPM(true)}>Yes</button>
              <button type="button" className={`ob-pill ${isPM === false ? 'ob-pill-active' : ''}`} onClick={() => setIsPM(false)}>No</button>
            </div>
            {isPM === false && (
              <input
                className="ob-input"
          ...
          <div className="ob-field">
            ...
            <div className="ob-pill-row">
              {EXP_OPTIONS.map(opt => (
                <button key={opt} type="button" className={`ob-pill ${experience === opt ? 'ob-pill-active' : ''}`} onClick={() => setExperience(opt)}>{opt}</button>
              ))}
            </div>
          </div>
          ...
          <div className="ob-field">
            ...
            <div className="ob-pill-row">
              {GENDER_OPTIONS.map(opt => (
                <button key={opt} type="button" className={`ob-pill ${gender === opt ? 'ob-pill-active' : ''}`} onClick={() => setGender(opt)}>{opt}</button>
              ))}
            </div>
          </div>
          ...
          <div className="ob-field">
            ...
            <select className="ob-select" value={country} onChange={e => setCountry(e.target.value)}>
        ...
        <button type="button" className="ob-skip" onClick={skip}>Skip for now</button>
      </div>
    </div>
  );
}
