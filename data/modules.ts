...
const QUIZZES_EN: QuizQuestion[][] = [
  [
    ...
    { q: "A team wants to update the app's navigation menu on iOS. Why can't this go live as instantly as a backend change?", opts: ["It requires App Store review before reaching users", "iOS doesn't support navigation changes", "It's actually a database change", "Navigation menus can never change"], correct: 0 },
    { q: "In the restaurant analogy, what does the database represent?", opts: ["The dining room", "The kitchen", "The pantry and reservation book — what's stored and remembered", "The waiter"], correct: 2 },
    { q: "Why does a 'resume watching' feature need a database?", opts: ["It stores your last watched position so it persists across sessions and devices", "It makes the video decode faster", "It's part of the frontend rendering", "It's only needed for offline playback"], correct: 0 },
  ],
  [
    ...
    { q: "Which HTTP method creates a brand new resource?", opts: ["GET", "POST", "DELETE", "PATCH"], correct: 1 },
    { q: "A request returns status 401. What does that mean?", opts: ["Server error", "Resource not found", "Unauthorized — the client isn't authenticated", "Bad request format"], correct: 2 },
    { q: "What does GET /api/products/42 return?", opts: ["A list of all products", "Product #42 only", "Creates product #42", "Deletes product #42"], correct: 1 },
  ],
  [
    ...
    { q: "Why does caching make a feed like Twitter's load instantly?", opts: ["The feed is pre-computed and stored in fast memory instead of freshly queried every time", "Twitter has fewer users than expected", "The frontend hides the loading time", "Caching disables the database"], correct: 0 },
    { q: "What does a load balancer do when it detects an unhealthy server?", opts: ["Shuts down the entire system", "Stops sending it traffic and routes to healthy servers instead", "Deletes the server's data", "Sends it more traffic to test recovery"], correct: 1 },
    { q: "What's the main tradeoff of choosing microservices over a monolith?", opts: ["More operational complexity in exchange for independent scaling and deployment", "Microservices are always slower to run", "Monoliths can't be deployed to the cloud", "Microservices remove the need for a database"], correct: 0 },
  ],
  [
    ...
    { q: "What is a design token?", opts: ["A component like a Button", "A named variable for a design decision, like a color or spacing value", "A type of API request", "A Figma plugin"], correct: 1 },
    { q: "Why does Server-Driven UI let a team run a flash-sale layout without an app store update?", opts: ["The server sends layout instructions the app renders on the fly, not hardcoded screens", "It bypasses app stores entirely for all future releases", "It only works on Android", "It requires no backend at all"], correct: 0 },
    { q: "Which tool typically holds the 'design side' of a design system, kept in sync with code?", opts: ["A spreadsheet", "A design tool like Figma", "The production database", "The CDN"], correct: 1 },
  ],
  [
    ...
    { q: "What does LCP (Largest Contentful Paint) measure?", opts: ["How long until the largest visible element finishes rendering", "Time until the first click", "Total page weight in KB", "Number of layout shifts"], correct: 0 },
    { q: "A good CLS (Cumulative Layout Shift) score is under what value?", opts: ["0.1", "1.0", "2.5", "100ms"], correct: 0 },
    { q: "What commonly causes a high CLS score?", opts: ["A fast server response", "Images without defined dimensions, or late-loading ads and fonts", "Using too much caching", "Using a CDN"], correct: 1 },
  ],
  [
    ...
    { q: "What is 'optimistic UI'?", opts: ["Waiting for server confirmation before updating anything", "Showing the expected result immediately, before the server confirms it", "A bright, colorful UI style", "Something only offline apps use"], correct: 1 },
    { q: "What's the main risk of optimistic UI?", opts: ["It's always slower than waiting", "If the server request fails, the UI has to roll back the premature update", "It requires no error handling at all", "It only works with local state"], correct: 1 },
    { q: "Why is keeping the cart count in sync across the header, cart page, and checkout considered hard?", opts: ["It isn't actually hard", "The same data must stay consistent across multiple views, and bugs are hard to reproduce", "It only affects the backend", "Databases don't support shopping carts"], correct: 1 },
  ],
  [
    ...
    { q: "Why is 'just rename the field' risky for a live API?", opts: ["It's never risky", "Old mobile versions and partners may still depend on the old field name and will break", "Field names don't matter to clients", "Renaming automatically updates every consumer"], correct: 1 },
    { q: "What's a typical deprecation timeline before removing an old API field?", opts: ["Immediately, no notice needed", "6–12 months, running old and new versions in parallel", "About 10 years", "There's no standard practice"], correct: 1 },
    { q: "Which kind of API change is considered backwards compatible?", opts: ["Renaming a field", "Removing a required parameter", "Adding a new optional field", "Changing a field's data type"], correct: 2 },
  ],
  [
    ...
    { q: "Why do authentication tokens (like JWTs) expire?", opts: ["To limit the damage if a token gets stolen", "Because servers can't store long strings", "It's a legal requirement everywhere", "To save storage space"], correct: 0 },
    { q: "Under GDPR/LGPD, what right must users have over their own data?", opts: ["None — companies own all user data", "The right to deletion and transparency about how it's used", "Only the right to view ads", "The right to sell their data to the company"], correct: 1 },
    { q: "What does a refresh token let an app do?", opts: ["Permanently disable authentication", "Get a new access token without forcing the user to log in again", "Delete the user's account", "Bypass HTTPS"], correct: 1 },
  ],
  [
    ...
    { q: "Why should staging mirror production as closely as possible?", opts: ["It doesn't really matter, staging is just for looks", "Bugs caught in staging are cheap; the same bugs in production are expensive", "Staging is only used for design review", "Production doesn't use real infrastructure"], correct: 1 },
    { q: "What's the fastest way to disable a broken feature without a new deployment?", opts: ["Roll back the entire codebase", "Turn off its feature flag", "Restart all servers", "Wait for the next scheduled release"], correct: 1 },
    { q: "What are the 'three pillars of observability'?", opts: ["Design, code, test", "Logs, traces, and metrics", "Frontend, backend, database", "Staging, production, local"], correct: 1 },
  ],
  [
    ...
    { q: "A system has a 0.1% error rate on 10 million daily requests. Roughly how many failed interactions is that per day?", opts: ["100", "1,000", "10,000", "100,000"], correct: 2 },
    { q: "What does 99.99% uptime ('four nines') allow, compared to 99.9% ('three nines')?", opts: ["The same downtime as three nines", "Far less downtime — about 52 minutes a year instead of 8.7 hours", "More downtime than three nines", "Downtime doesn't change with more nines"], correct: 1 },
    { q: "Why should PMs care about P95/P99 latency, not just the average?", opts: ["They shouldn't — averages tell the whole story", "P95/P99 reveal the experience of the slowest users, often a meaningful and valuable segment", "P95/P99 only matter for mobile apps", "They're the same as P50 in practice"], correct: 1 },
  ],
];
